...
raaArc* initArc(raaArc* pArc, raaNode* pNode0, raaNode* pNode1, float fSpringCoef, float fIdealLen, float fNonLinearFactor)
{
	if(pArc)
	{
		...
		pArc->m_bFlowDirection = pNode0->m_fMass > pNode1->m_fMass;
		pArc->m_fNonLinearFactor = fNonLinearFactor;
